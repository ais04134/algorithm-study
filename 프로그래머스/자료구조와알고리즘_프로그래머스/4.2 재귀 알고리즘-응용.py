# 재귀적인 방식으로 함수를 호출하면, 값이 커질수록 불필요한 함수를 반복적으로 호출하는 경우가 생기므로 효율성이 떨어진다.
...
def solution_ver2(L, x, l, u): # 제출시 효율성이 위에 코드보다 떨어진다고 나온다. # 손으로 작성해보니 그렇네! 이해완료
    if u == l:
        return -1
    mid = (l+u)//2
    if x == L[mid]:
