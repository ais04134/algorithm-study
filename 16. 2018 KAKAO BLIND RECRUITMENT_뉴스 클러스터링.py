...
def positive(x):
    return x.isalpha() # 영문자만을 걸러내기 위한 함수

def solution(str1, str2):
    str1 = str1.upper()
    str2 = str2.upper() # 1. 대소문자 통일
    str1_list = []
    str2_list = []
    empty_set = 0
    union = 0  # 2. 변수 값 선언
    ...
    for name in str1_list:
        ...
        union += max(str1_list.count(name), str2_list.count(name))

        str1_list = list(filter(name.__ne__, str1_list))
        str2_list = list(filter(name.__ne__, str2_list)) # 리스트에서 중복되는 값 제거

    union += len(str2_list)

    if union == 0 and empty_set == 0: return  1*65536
